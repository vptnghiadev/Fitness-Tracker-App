...
public class workout_summary extends FragmentActivity {
    ...
    private void checkSavedData(String objectId) {
        ParseQuery<ParseObject> query = ParseQuery.getQuery("Workout");
        query.getInBackground(objectId, (object, e) -> {
            if (e == null) {
                // Nếu lấy được dữ liệu, in ra thông tin đối tượng
                Log.d("CHECK", "Workout Object ID: " + object.getObjectId());
                Log.d("CHECK", "Calories: " + object.getDouble("calories"));
                Log.d("CHECK", "Duration: " + object.getString("duration"));
                Log.d("CHECK", "Workout Type: " + object.getString("workoutType"));
            } else {
    ...
    private void submitWorkout(String time, String workoutTypeString) {
        ...
        workout.saveInBackground((e) ->{
            if (e == null) {
                Log.d("CREATE", "Created object: " + workout.getObjectId());
                checkSavedData(workout.getObjectId());
            } else {
